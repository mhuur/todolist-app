...
3. **Fournisseurs** (`tab:"crm"` → `rCRM` ; ⚠️ clé d'onglet et état `S.crm*` gardent le nom historique `crm`) — 2 vues `S.crmView` : **Fournisseurs** / **Produits**. Fournisseur = `{name, products[], contacts[]}` ; `S.crm.preferred`. Sync Google Contacts (People API, scope `contacts.readonly`, bouton « Resync », matching via `crmMatchOrg`/`contactAlias`). **Recherche étendue aux contacts** (août 2026) : en vue Fournisseurs, la recherche matche aussi nom/email/téléphone des contacts (`ctHit`), et la carte du fournisseur s'ouvre alors automatiquement. **Contacts éditables** (helpers `crmCt*`, transient `crmCtEd`) : ajout (« ＋ Ajouter un contact »), modification (crayon) et suppression par contact, formulaire inline `.ctform` (nom/email/tél, Entrée=OK, Échap=annuler). Un contact Google (`src:'g'`) édité passe `src:'m'` en gardant son `gId` (la resync dédoublonne par `gId` au lieu de le réimporter) ; un contact Google supprimé est mémorisé dans `S.crm.contactHide[gId]` pour ne pas revenir à la resync (les contacts manuels, eux, survivent nativement : `gcRebuild` ne reconstruit que les `src:'g'`). Autocomplete produits via les helpers partagés `acOpen`/`acFilter` (`ac*`, hérités de l'ancien onglet Tâches — toujours vivants, ne pas casser).
