...
> **Direction visuelle (décidée 2026-07-01) : migration vers le thème _clair Ekopak_.** L'app passe du néon sombre au design system Ekopak — fonds clairs (`--surface-card`/`--neutral-50`), bleu profond `#0f3f93`, dark blue `#1a2853`, accents light-blue `#81d0f5`/corail/lime, ombres bleutées douces, police Montserrat. Tokens de référence dans `../ekopak-design/tokens/colors.css` — ⚠️ **ce dossier n'est PAS déployé** (prod = `index.html` seul) : pas de `<link>` ni d'asset externe (le logo Ekopak officiel est un PNG non déployable → on recrée en **SVG inline**), tout doit rester auto-contenu. *(L'ancien aperçu `revision.html` a été supprimé et n'a jamais été committé — ne plus s'y référer.)* **Migration écran par écran, en cours.** Déjà porté : le **menu latéral** (navy `#1a2853`, onglet actif deep-blue, logo goutte SVG inline, avatar à initiales — voir `.sidebar`/`.sb-logo`/`.sb-btn`/`.sb-avatar` dans le `<style>`). **Le reste du contenu est encore en néon sombre** — ne pas mélanger les deux langages dans un même écran (le sidebar étant déjà sombre, son passage en navy Ekopak reste cohérent avec le contenu néon). Les règles « néon » ci-dessous restent la référence pour un écran encore sombre ; pour un écran migré, suivre le thème clair Ekopak.
