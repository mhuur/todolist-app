...
**MyTodo** — app web personnelle (todo + deadlines + CRM fournisseurs/produits + CRM projets/technos + Notes). **Mono-utilisateur** (Google Auth, doc Firestore `users/{uid}`). **Production**, page statique servie depuis `main` (GitHub Pages / équivalent).
...
**Sidebar** (tableau `tabs`, dispatch `S.tab` → renderer). `sT(k)` change d'onglet. **Onglet par défaut : Projet.** Onglets : Projet, **Planning**, Temps, Fournisseurs, Projets, Prix, Notes, Corbeille.
...
4. **Projets** (`tab:"projets"` → `rPC` ; ⚠️ ne PAS utiliser la clé `"projects"`, redirigée vers `"flux"` par le routing legacy) — **CRM projets / technos**, indépendant de l'écran Projet (tâches) et du legacy `S.projects`. Modèle persisté `S.projCrm={technos:[type…], projects:[{id, number, name, type, sharepoint, technos:[{id, type, lignes, capacite}]}]}` — `type` projet ∈ `PC_TYPES` (`WaaS`/`Capex`/`Rental`, vide autorisé). **`pcNorm` (appelé aux 3 points de chargement) porte la migration idempotente de l'ancien modèle de techno `{name, debit}` → `{id, type, lignes, capacite}`** : ne pas la retirer, des données prod sont dans l'ancien format. 2 vues `S.pjView` (`'proj'`/`'tech'`, transient) + recherche `S.pjQ` (transient ; filtre n°, nom **et** type de techno), gabarit `.c2wrap` du CRM Fournisseurs réutilisé tel quel + styles propres (`.prow`, `.pc-back`/`.pc-card`, `.fld`, `.trow`, `.splink`).
   - **Vue Projets = une ligne par projet** (`.prow`, triée n° décroissant, sans n° à la fin) : n°, nom, badge type, résumé des technos (`pcTechLabel` → « UF 50 · ROSP 35 »), icône lien (**l'URL n'est jamais affichée en clair** ; rendue seulement si `pcSpOk` — l'URL commence par http(s)), corbeille. **Clic sur la ligne → carte projet en modale** (`rPCCard`, état transient `pcOpenId`) : n° + nom (`oninput` sans re-render), type en `.seg` 3 boutons (recliquer le type actif le retire), champ lien (`onchange` + re-render pour rafraîchir l'icône) avec l'icône d'ouverture à côté, puis la section **Technos** en 3 colonnes **toutes éditables** — Type / Lignes / Capacité (`oninput` sans re-render). Ligne techno **réordonnable par drag** via la poignée seule (`pcTDS`/`pcTDO`/`pcTDrop`, drop = insertion avant la ligne cible ; `draggable` sur le grip, PAS sur la ligne, sinon la sélection de texte dans les inputs casse). Créer un projet ouvre directement sa carte.
...
6. **Notes** (`tab:"feedback"` → `rFB`) — Notes & remarques à transmettre à Claude (`S.feedback.items[]`), validables/supprimables, bouton « Copier les actions en cours » (`fbCopy`).
7. **Corbeille** (`tab:"trash"` → `rTR`) — éléments supprimés (`S.trash[]`), restaurer / vider.
