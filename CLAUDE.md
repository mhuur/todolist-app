...
4. **Projets** (`tab:"projets"` → `rPC` ; ⚠️ ne PAS utiliser la clé `"projects"`, redirigée vers `"flux"` par le routing legacy) — **CRM projets / technos**, indépendant de l'écran Projet (tâches) et du legacy `S.projects`. Modèle persisté `S.projCrm={technos:[nom…], projects:[{id, number, name, sharepoint, technos:[{name, debit}]}]}` (normalisé par `pcNorm` aux 3 points de chargement). 2 vues `S.pjView` (`'proj'`/`'tech'`, transient) + recherche `S.pjQ` (transient), même gabarit que Fournisseurs (`.c2wrap` réutilisé tel quel ; styles propres `.pc-num`/`.pc-sp`/`.pc-debit`/`.splink`). **Vue Projets** : cartes triées n° décroissant (sans n° à la fin) ; en-tête = n° projet + nom éditables inline (`oninput` sans re-render), badge nb technos, icône lien SharePoint (rendue seulement si l'URL commence par http(s)), corbeille ; corps = champ lien SharePoint (`onchange`+re-render pour rafraîchir l'icône), autocomplete « Ajouter une techno » (créer-si-absent), lignes techno avec **débit** texte libre (`oninput` sans re-render). **Vue Technos** : catalogue `S.projCrm.technos` ; carte = rename `onchange` (`pcRenTech` propage aux projets + clés `S.ex`), autocomplete « Ajouter un projet » (créer-si-absent), lignes projet avec débit. Suppressions via modale thémée (`pcDelC` transient). Helpers préfixés `pc*` ; autocompletes custom `pcTechOpen`/`pcProjOpen` (PAS `acOpen`, qui rendrait la liste legacy sur un id contenant `proj`). Expansion : `S.ex["pc_p_<id>"]` / `S.ex["pc_t_<slug>"]`.
