...
**Sidebar** (tableau `tabs`, dispatch `S.tab` → renderer). `sT(k)` change d'onglet. **Onglet par défaut : Projet.** Onglets : Projet, **Planning**, Fournisseurs, Projets, Prix. *(Temps masqué du rail en août 2026 — voir point 2.)* **La Corbeille n'est PAS dans le rail** : elle vit dans le **menu du rond d'initiales** en bas de sidebar (`avMenu` transient, ouvert au **clic** — plus au hover —, `avToggle`/`avClose`/`avGo`, fermé par le listener `document click` global ; contient nom + état de sync + Corbeille + Déconnexion).
